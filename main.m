...
datos = {'Sol' , 1.989e30 , 695508 , 0 , 0 , 0 ;...
         ...
         'Nave Espacial' , 28800 , 2e-3 , 149597870.691+100000 , 7 , 29.78+1 ;... % la nave
...
t_galletazo = floor(Nk/2); % el subintervalo en el cual se aplica el galletazo 0 < t_g < Nk
...
curvas = gobjects(1,nc);

curvas(1) = plot3(cuerpos(1).tryct(1,1),cuerpos(1).tryct(1,2),cuerpos(1).tryct(1,3),'o');
hold on
grid on

for k = 2:nc
    curvas(k) = plot3(cuerpos(k).tryct(1,1),cuerpos(k).tryct(1,2),cuerpos(k).tryct(1,3),'.'); 
end
legend(legends);
for t = 2:Nk
   for k = 1:nc
      set(curvas(k),'xdata',cuerpos(k).tryct(1:t,1),'ydata',cuerpos(k).tryct(1:t,2),'zdata',cuerpos(k).tryct(1:t,3));
      if hay_galletazo == 1 && t == t_galletazo
         legends(nc+1) = 'Galletazo';
